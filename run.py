...
if __name__ == '__main__':
    ...
    parser.add_argument('--moe_loss_weight', type=float, default=1, help='weight for moe auxiliary loss')
    ...
    # 获取当前时间，格式为：月日_时分
    # 放在循环外，确保整批次实验使用同一个时间前缀
    curr_time = time.strftime("%m%d_%H%M", time.localtime())

    if args.is_training:
        for ii in range(args.itr):
            # setting record of experiments
            # 统一拼接格式：包含了 curr_time，删除了缺失的 class_strategy
            setting = '{}_{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_fc{}_eb{}_dt{}_dp{}_bs{}_{}_{}'.format(
                args.model_id,
                args.model,
                args.data,
                args.features,
